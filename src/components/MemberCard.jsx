    ...
    return (
        <div className={`group relative bg-white/60 backdrop-blur-xl rounded-2xl p-4 lg:p-5 border border-white/40 shadow-sm transition-all duration-300 overflow-hidden ${hasError ? 'border-red-200 bg-red-50/40' : 'hover:border-indigo-200/50 hover:bg-white hover:shadow-xl hover:shadow-indigo-500/5 hover:scale-[1.01]'}`}>
            ...
            <div className="flex flex-col w-full relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                            {isEditingName ? (
                                ...
                                <div
                                    ...
                                    className={`flex items-center gap-2 truncate ${isLocked ? 'cursor-default' : 'cursor-text group/name'}`}
                                >
                                    ...
                                    {!isLocked && <Edit3 className="w-3.5 h-3.5 text-indigo-300 drop-shadow-sm flex-shrink-0" />}
                                </div>
                            )}
                            {/* V7.5 Summary Line - Replaces 'Active Member' */}
                            ...
                            {/* Liquid/Fluid Bar Indicator representing input scale */}
                    ...
                    {isMonthlyMode && (
                        <div className="flex items-center gap-2 flex-shrink-0 translate-y-[-2px]">
                            {isEditingAbsent && !isLocked ? (
                                ...
                                <div
                                    onClick={() => !isLocked && setIsEditingAbsent(true)}
                ...
                <div className={`w-full flex flex-col gap-4 origin-top transition-all duration-300 max-h-0 opacity-0 overflow-hidden mt-0 ${!isLocked ? 'group-hover:max-h-[800px] group-hover:opacity-100 group-hover:overflow-visible group-focus-within:max-h-[800px] group-focus-within:opacity-100 group-focus-within:overflow-visible group-hover:mt-4 group-focus-within:mt-4' : ''}`}>
